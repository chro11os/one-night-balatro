...
impl Card {
    pub fn new(id: i32, x: f32, y: f32) -> Self {
        Self {
            ...
            scale: 0.0, target_scale: 1.0,
            rotation: 0.0, target_rotation: 0.0, rot_velocity: 0.0, tilt: 0.0,
    ...
    pub fn update_anim(&mut self, dt: f32, total_time: f32) {
        ...
        if self.is_dragging {
            ...
            self.target_rotation = (total_time * 6.0).sin() * 0.05;
        } else {
            // FIX: Reset scale to 1.0 when not hovered!
            self.target_scale = 1.0;
            self.target_rotation = 0.0;
        }

        let rot_force = (self.target_rotation - self.rotation) * 1200.0;
